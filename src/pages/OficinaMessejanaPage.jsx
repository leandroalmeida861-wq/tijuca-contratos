...
const EMPTY_FILTERS = { search: '', fornecedor: '', produto: '', placa: '', motorista: '', destino: '', status: '' };
...
function ReportsTab({ dashboard, period, onPeriod, can, onError }) {
  ...
  async function exportReport(format) {
    ...
    try {
      ...
      const rows = entradas.map((row) => ({ Tipo:'Entrada', Data:dateTime(row.data_entrada), NF:`${row.nf_numero}/${row.nf_serie}`, Fornecedor:row.fornecedor_nome, Produto:row.produto_nome, Destino:'', Peso:Number(row.peso_nf), ValorUnitario:Number(row.valor_unitario||0), ValorTotal:Number(row.valor_total_nota||0), Saldo:Number(row.saldo_disponivel), Status:statusLabel(row.status_saldo) })).concat(saidas.map((row) => ({ Tipo:'Saída', Data:dateTime(row.data_saida), NF:`${row.nf_saida_numero}/${row.nf_saida_serie}`, Fornecedor:row.fornecedor_nome, Produto:row.produto_nome, Destino:row.destino, Peso:Number(row.peso_saida), ValorUnitario:0, ValorTotal:0, Saldo:Number(row.saldo_apos_saida), Status:statusLabel(row.status_registro) })));
