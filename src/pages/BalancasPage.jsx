...
function PortariaTab({ rows, options, unidade, balanca, can, loading, reload, setError, setMessage }) {
  ...
  async function submit(event) {
    event.preventDefault();
    setError('');
    const validation = validatePortariaForm(form);
...
function findDuplicateRecebimentoRows(rows, payload, editingId, options = {}) {
  ...
  const nfSerie = normalizeNfSeries(payload.nf_serie);
  ...
  return (rows || []).find((row) => {
    ...
    if (normalizeNfSeries(row.nf_serie) !== nfSerie) return false;
...
function validatePortariaForm(form) {
  const missing = [];
  const fields = {};
...
function normalizeNfSeries(value) {
