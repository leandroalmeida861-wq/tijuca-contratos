...
as $$
declare
  new_nf text := regexp_replace(coalesce(new.nf_numero, ''), '\D', '', 'g');
  new_cnpj text := '';
  new_nome text := '';
  duplicate_id uuid;
begin
  ...
  where r.id <> new.id
    ...
    and regexp_replace(coalesce(r.nf_numero, ''), '\D', '', 'g') = new_nf
    and (
...
before insert or update of nf_numero, fornecedor_id, status on public.recebimentos
